...
@IBDesignable
public class CurvedLabel : UILabel
{
    @IBInspectable
    public var anchorAngle : CGFloat = π { didSet { setNeedsDisplay() } }
    ...
    func findFontPointSize(proposedPointSize: CGFloat) -> CGFloat
    {
        ...
        preconditionFailure("Could not create font")
    }
    ...
    override public func drawRect(rect: CGRect)
    {
        ...
        if let font = UIFont(name: self.font.fontName, size: fontPointSize)
        {
            ...
            if let context = UIGraphicsGetCurrentContext()
            {
                ...
                {
                    position = CGPointMake(CGRectGetMidX(self.bounds), self.bounds.size.height - (font.ascender + font.pointSize / 3))
                }
                else
                {
                    position = CGPointMake(CGRectGetMidX(self.bounds), -font.descender + margin)
                }
